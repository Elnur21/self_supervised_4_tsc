...
from encoders.lite import LITE
...
class MODEL:
    ...
    def build_model(self):
        ...
        elif self.encoder_name == 'lite':
            encoder = LITE(input_shape=input_shape,n_dim=self.n_dim)
        ...
        tf.keras.utils.get_custom_objects()['temp'] = my_loss
    ...
    def fit(self,xtrain,xval):
        ...
        for _epoch in range(self.epochs):
            ...
            hist = self.model.fit([ref_train,pos_train,neg_train],np.zeros(shape=ref_train.shape),
                                   epochs=1,batch_size=self.batch_size,verbose=True)
